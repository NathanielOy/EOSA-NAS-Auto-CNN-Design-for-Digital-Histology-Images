...
class BaseEBOA(EOSANASEvaluationStrategy):
    """
    ...
    def __init__(self, root_algo_paras=None, evaluated_solutions=None, eboa_paras=None, model_rates=None, pos_ids=None, ssp=None, search_paras=None, cnn_configs=None, num_classes=None, bounds=None):
        EOSANASEvaluationStrategy.__init__(self, root_algo_paras, evaluated_solutions, pos_ids, search_paras, cnn_configs, num_classes, bounds)
        self.epoch = eboa_paras["eosa_epoch"]
        self.pop_size = eboa_paras["problem_size"]
        ...
        self.solutions=[]
        self.S =self._evaluate_solution__()
        self._cache_solutions_state__(0) #Write result of all initial solutions
        #print('This is my lenght '+str(len(self.S)))
        self.a_scatter=None
        self.unpack_pop=[]
        self.ssp=ssp
    ...
    def _train__(self):      
        ...
        PE=[(i,self._create_solution__(-1, ssp=self.ssp)) for i in range(1)] # population of infected pathogens in environment PE
        ...
        for i in range(self.epoch):
            ...
            if len(self.I) > 1:
                ...
                qsize=qsize if len(self.I) > qsize else len(self.I)-1
            ...
            print("Epoch = {}, Best Fit = {}, Best position = {} ".format(i+1, gbest[self.ID_FIT], gbest[self.ID_POS]))
            
            '''
            Write result of all solutions after each epoch
            '''
            self._cache_solutions_state__(i+1)
    
        #self._show_all_fits()
        #print(self.solutions)
        return gbest[self.ID_POS], self.solutions
    
    def _cache_solutions_state__(self, i):
        save_results_dir='./epoch_'+str(i)+'_'
        params={"save_results_dir":save_results_dir, "eosa_epoch": self.epoch, 'problem_size':self.pop_size}
        pr=ProcessResult(params=params)
        pr._save_results__(solutions=self.sols_config_and_perfromances, epoch_record=True)
    ...
    def _quarantine_infectecd_individuals__(self, qsize=0):
        for i in range(qsize):
            self.Q.append(deepcopy(self.I[i]))
    ...
    def _addback_recovered_2_susceptible__(self, recovered=None):
        ...
        for r in recovered:
            ...
            for s in self.S:
                ...
                s_individual[0] = np.array(s_individual[0])
                r_individual[0] = np.array(r_individual[0]) 
                #(s_individual[0] == r_individual[0]).all(
                if ( np.array_equal(s_individual[0], r_individual[0]) ) and s_indx is None: #.all()
    ...
    def _rebirth_2replace_dead_in_susceptible__(self, dead=None): 
        ...
        for d in dead:
            ...
            for s in self.S:
                s_indx, s_individual=s
                s_individual[0] = np.array(s_individual[0])
                d_individual[0] = np.array(d_individual[0]) 
                if np.array_equal(s_individual[0], d_individual[0]) and s_indx is None:
                    new_solution=self._create_solution__(d_indx, ssp=self.ssp, minmax=self.MIN_MAX_INFECTED_SOL)
    ...
    def _boost_imunity_for_infection_escpades__(self, x, indvd_change_factor, gbest):
        ...
        escape_fit_infected = self._reevaluate_solution__(self.ssp, solution=escape_infected_ind, minmax=self.MIN_MAX_INFECTED_SOL)
    ...
    def _weaken_imunity_of_infected__(self, x, indvd_change_factor, gbest):
        ...
        infected_ind= v
        fit_infected = self._reevaluate_solution__(original_index, self.ssp, solution=infected_ind, minmax=self.MIN_MAX_INFECTED_SOL)
...
'''
v = np.abs((np.random.rand()* indvd_change_factor) * (gbest[ID_POS]- deepcopy(ix[ID_POS])))
...
#infected_ind = self.domain_range[0] if (infected_ind < self.domain_range[0]).all() else infected_ind
#infected_ind = self.domain_range[1] if (infected_ind > self.domain_range[1]).all() else infected_ind
