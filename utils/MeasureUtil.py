...
"""
class MeasureClassification(object):
    def __init__(self, y_true=None, y_pred=None, number_rounding=3):
        ...
        self.std_2, self.median=None, None
    ...
    def best_score(self, solutions):
        temp = np.max(solutions)  # 
        self.best=temp
    
    def mean_score(self, solutions):
        temp = np.mean(solutions)  #self.solutions.np.median(x)
        self.mean=temp
    
    def std_score(self, solutions):
        temp = np.std(solutions) #
        self.std=temp
    
    def worst_score(self, solutions):
        temp = np.min(solutions)  # 
        self.worst=temp
        
    def median_score(self, solutions):
        temp = np.median(solutions)  # 
        self.median=temp
    
    def _eosa_fit_accuarcy__(self, solutions):
        self.best_score(solutions)
        self.mean_score(solutions)
        self.std_score(solutions)
        self.worst_score(solutions)
        self.median_score(solutions)
        return [self.best, self.mean, self.median, self.std, self.worst]
    
    def _eosa_fit_loss__(self, solutions):
        self.best_score(solutions)
        self.worst_score(solutions)
        self.median_score(solutions)
        return [self.best, self.median, self.worst, self.std]
